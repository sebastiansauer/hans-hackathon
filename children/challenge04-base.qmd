...
2. *Pivotieren* Sie den Datensatz in das Langformat. Aber nehmen Sie `idvisit` vom Pivotieren aus; d.h. Sie nehmen alle Spalten bis auf `idvisit` in das Pivotiren auf. Es sollen also drei Spalten resultieren: `idvisit`, `name` und `value`.

3. *Begrenzen* Sie beim Pivotieren die Spalten auf die Spaltentypen mit dem Namen vom Typ `subtitle` sowie `idvisit`; d.h. Sie pivotieren nur diese genannten Spaltentypen.
